...
export default {
  ...
  methods: {
    ...
    scaleEvent(scale) {
      // 渲染中 不让缩放 也不让重绘
      if (this.pageRenderedNum != this.totallPage || this.totallPage === 0)
        return;
    ...
    scaleCanvas(scale) {
      ...
      this.canvasEles.forEach((canvas) => {
        ...
        var unscaledViewport = this.page.getViewport({ scale: 1 });
        var scale = Math.min(
      ...
      for (let pageNum = 1; pageNum <= len; pageNum++) {
        ...
        this.pdfDoc.getPage(pageNum).then(function (page) {
          ...
          renderTask.promise
            ...
            .then((textContent) =>
              that.textRerender.call(that, pageNum, textContent)
            )
    ...
    renderSinglePage(canvas, pageNum) {
      ...
      this.pdfDoc.getPage(pageNum).then(function (page) {
        ...
        renderTask.promise
          ...
          .then((textContent) =>
            that.textRender.call(that, canvas, pageNum, textContent)
          );
      });
    },
    ...
    getCanvasCSSWH() {
      ...
      const width = (0, this.roundToDivide)(
        this.viewport.width * outputScale.sx,
        sfx[0]
      );
      const height = (0, this.roundToDivide)(
        this.viewport.height * outputScale.sy,
        sfy[0]
      );
      ...
      if (this.pixelRatio !== 1)
        this.transform = [this.pixelRatio, 0, 0, this.pixelRatio, 0, 0];
      ...
      console.log(this.curCanvasCSSWh);
