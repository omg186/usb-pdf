  ...
  setup(scope, {}) {
    ...
    const loginNode = (
      <div class="tab-item-box login">
        <Form layout="vertical" model={values} onFinish={submitForm}>
          <Form.Item
            name="username"
            rules={[{ required: true, message: "请输入登入账号！" }]}
          >
            <Input
          ...
          >
            <Input.Password
              prefix={<LockOutlined style="color:rgba(0,0,0,.44)" />}
              onChange={(ev) => (values.password = ev.target?.value)}
              placeholder="请输入密码"
            />
          </Form.Item>
          <Form.Item>
    ...
    const registorNode = (
      <div class="tab-item-box register">
        <Form layout="vertical" model={values} onFinish={submitForm}>
          ...
          >
            <Input
              suffix={<UserOutlined style="color:rgba(0,0,0,.44)" />}
              onChange={(ev) => (values.username = ev.target?.value)}
              placeholder="输入手机号"
            />
          </Form.Item>
          ...
          >
            <Input.Password
              onChange={(ev) => (values.password2 = ev.target?.value)}
              placeholder="输入确认密码"
            />
          </Form.Item>
          <Form.Item>
    ...
    return () => (
      <div class="login-wrap">
        ...
        <div class="left-banner d-flex align-items-center justify-content-center">
          BANNER
        </div>
        ...
        <div
          class="login-close"
          onClick={() => ipcRenderer.invoke(LOGIN_CLOSE)}
        >
          <CloseOutlined />
        </div>
      </div>
